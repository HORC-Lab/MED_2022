function x_slice = sim_ms_to_ms_compliant(init_conds,control_input,des_steps)
% function x_slice = sim_ms_to_ms_compliant(init_conds,control_input,des_steps)

% Description:
% This function simulates the system from MS to MS for a specific number
% of steps (<=2), using the given initial conditions and control inputs. In this
% code, no controller is applied. The system is simply being simulated
% using the given initial conditions and control inputs. This is useful to
% linearize the return map and hence derive the appropriate LQR controller.
% For information, the reader is referred to the technical report by
% Karakasis: "Report for 3D Dual SLIP Model Analysis".

% INPUTS:
% init_conds:       initial conditions to be utilized for the simulation
% control_input:    control input to be utilized for the simulation
% des_steps:        desired number of steps for the simulation (not greater than 2)

% OUTPUTS:
% x_slice: The MS state derived at the last step of the simulation

%================================================================================================%
% Stop simulation in case the user requested a desired number of steps
% greater than 2.
if des_steps > 2
    x_slice = [];
    return;
...
%-------------------------------------------------------------------------%
%% Initializations
phi = control_input(1);                                             % Set the lateral touchdown angle
theta = control_input(2);                                           % Set the forward touchdown angle
z_th = l_0*sin(theta);                                              % Set Threshold height at which Touchdown takes place (determined by angle of attack)
k_A = control_input(3);                                             % Set the stiffness of leg in support
k_B=k_A;                                                            % Both legs share the same stiffness
%-------------------------------------------------------------------------%
...
%A walking step is defined between two subsequent midstance (MS) events (Lie et al.)
...
%================================================================================================%
%% Simulation Code
%Basically for every gait event we execute a  while loop. In the first two,
% we utilize single support dynamics and we keep iterating until a
% Midstance and a Touchdown gait events take place, respectively. For the last
% two, we utilize double support dynamics and keep iterating until a Lowest
% Height and a Lift Off gait events take place, respectively. In all four
% loops, the iteration stops immediately if the maximum number of iterations
% (i_th) is reached.
%-------------------------------------------------------------------------%
%% While loop for Desired number of steps
while(steps < des_steps)                                            % Outer loop executed for every step. "des_steps" specifies the desired number of steps
    MS_frames = [];                                                 % In this vector we will store all time instances of every Midstance event encountered during the following iteration
    ...
    while (1)                                                       % Inner loop for ISS phase until MS
        ...
        MS_frame=find( (abs(z_c_dot)<thld_vel) & (z_c > z_th) & (sol_i(:,2)<l_0+thld_l) );  
        ...
        if (~isempty(MS_frames) && isempty(MS_frame)) || ~isempty((find(sol(:,2)>l_0+thld_l))) || i_i>i_th
            break;
        end
    end
    %-------------------------------------------------------------------------%
    if(isempty(MS_frames))                                          % Check whether the condition for the Midstance event was satisfied for any time instant during the iteration
        ...
        x_slice = [10^3 10^3 10^3 10^3 10^3];                       % Return an extremely large value to indicate that the system failed
    ...
    steps = steps + 1;                                              % A walking step is defined between two subsequent midstance (MS) events (Lie et al.)
    ...
    while (1)                                                       % Inner loop for ISS phase until TD
        ...
        % the leg in support exceeds the natural length beyond the allowed
        % threshold, or if the maxinum number of iterations was reached.
        if ( (~isempty(TD_frames) && isempty(TD_frame)) || ~isempty((find(sol(:,2)>l_0+thld_l))) || i_i>i_th )
            break;
        end
    end
    %-------------------------------------------------------------------------%
    ...
    %The above variables were determined for the whole duration of the simulation
    %in order to find directly the time instant of the most accurate event wrt to the total time
    TD_frame = TD_frames(find(abs(z_c(TD_frames)-z_th)==min(abs(z_c(TD_frames)-z_th)),1,'first'));      %Determine which frame amongst the Touchdown frames is the most accurate and define it as the Touchdown frame
    %-------------------------------------------------------------------------%
    ...
    odefun = odeFunction(f_ds,vars);                                % Convert symbolic expressions to function handle for ODE solvers
    %================================================================================================%
    ...
    while (1)                                                       % Inner loop for DS phase until LH
        [t_i,sol_i] = ode113(odefun,[0 max_step_size],init_conds,opts);     %Solve the system's equations for the DS Phase using the specified ICs for 0.01 secs with the error tolerances "opts" specified earlier
        t_total=[t_total(1:end-1);t_i+t_total(end)];                % Adjust the local time vector, so that it corresponds to the total time elapsed and append it to the total time vector
        sol=[sol(1:end-1,:);real(sol_i)];                           % Append the local solutions for the state to the total state solutions vector
        init_conds=[sol(end,1); sol(end,2); sol(end,3); sol(end,4); sol(end,5); sol(end,6); sol(end,7); sol(end,8); sol(end,9); sol(end,10)];    %Set the last state of the system for this iteration, as ICs for the next one
        ...
        % threshold, or if the maxinum number of iterations was reached.
        if ((~isempty(LH_frames) && isempty(LH_frame)) || ~isempty((find(lA>l_0+thld_l))) || ~isempty((find(lB>l_0+thld_l))) || i_i>i_th)
            break;
        end
    end
    %-------------------------------------------------------------------------%
    ...
    while (1)                                                       % Inner loop for DS phase until LO
        ...
        % the legs in support exceeds the natural length beyond the allowed
        % threshold, or if the maxinum number of iterations was reached.
        % Finally, check whether the height of the foot in support has any positive
        % values, meaning that it is floating on the air.
        if ( ( ~isempty(LO_frames) && isempty(LO_frame) ) || ~isempty((find(lA>l_0+thld_l))) || ~isempty((find(lB>l_0+thld_l))) || i_i>i_th || ~isempty(find(sol_i(:,1)>0)))
            break;
        end
    end
    %-------------------------------------------------------------------------%
    ...
    if(isempty(LO_frames))                                          % Check whether the condition for the Lift Off event was satisfied for any time instant during the iteration
        ...
        x_slice = [10^3 10^3 10^3 10^3 10^3];                       % Return an extremely large value to indicate that the system failed
        return;                                                     % In case no Lift Off events took place the simulation is terminated, the program counter exits from the "steps" iteration and goes directly to the Plotting Section
    end
    ...
    theta_1_B = atan2( (y_B_f-real(y_c)),(x_B_f-real(x_c)) );
    ...
    %Initial Support dynamics can begin their implementation exactly at
    %Lift Off.
    ...
    %-------------------------------------------------------------------------%
    % Sign-alternating forward touchdown angle at each step.
    phi = -phi;
    %================================================================================================%
    %% Analysis of the system equations that will be simulated using the "ode" step integrator during SSS for compliant terrain
    syms x(t) [10 1] real;                                          % Define the full state of the system
    %----- Equations for the Single Support Phase -----%
    ...
    x_A_f = x_B_f;
    y_A_f = y_B_f;
end         %Initiate once again the system from Single Support